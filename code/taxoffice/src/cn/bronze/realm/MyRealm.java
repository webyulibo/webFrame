...
public class MyRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
		...
		//permissions.add("sysauth");
		
		//获取用户名
		String username=(String) principalCollection.getPrimaryPrincipal();
		
		//根据用户名，查出对应的对象
		TUser user=adminUserManagerService.loginCheckOn(username);
		...
		if(authids!=null&&authids.size()>0){
			//根据权限id集合，获取所有权限
			authoritykeys=adminAuthService.getauthkeysbyauthids(authids);
			if(authoritykeys!=null&&authoritykeys.size()>0){
				
				for (int i = 0; i < authoritykeys.size(); i++) {
					permissions.add(authoritykeys.get(i));
				}
				
			}
			else{
				System.out.println("不存在对应的权限集合");
			}			
		}
		else{
			System.out.println("该角色下不存在权限");
		}				
...
//		adminUserManagerService.l
//		String password="admin";
//		SimpleAuthenticationInfo info=new SimpleAuthenticationInfo(username,password,getClass());
