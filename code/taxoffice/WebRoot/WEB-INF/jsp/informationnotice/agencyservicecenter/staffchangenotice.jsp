...
<body>
	<div class="container-f">
	  	 <div class="box-title clearfix tab-title"> <!-- 标题 -->
			...
			<div style=" float:left;margin-left:20px;">
				<a href="<%=basePath%>adminmessagecontroller/findacceptmessagepageofagencyservicecenterbycondition.htm?depcode=${depcode}&currentPage=${currentPage}&title=${title}&sendpeople=${sendpeople}&status=${status}&senddepname=${senddepname}" class="btn btn-primary ">返回</a>
		   </div>
		</div>
		...
		<fieldset class="search" style="margin-top:20px;">
			...
			<tr>
				...
				<td colspan="3" height="140px"><textarea id="usedep" style="width:100%;height:100%" readonly>${tMobilizepeople.usedep }</textarea></td>
				 ...
				 <c:if test="${tMobilizepeople.originaldep=='机关生活服务中心' }"> 
				 <td width="130px" height="140px"><center>使用部门办理情况：</center></td>
			...
			<tr>
				...
				<c:if test="${tMobilizepeople.status==false }"> 
				<td width="130px" height="140px"><center>机关服务中心办理情况：</center></td>
				<td colspan="3" height="140px"><textarea id="servicedep" style="width:100%;height:100%" >${tMobilizepeople.servicedep }</textarea></td>
				 </c:if>
	...
	<input type="hidden" id="tMobilizepeople-originaldep" value="${tMobilizepeople.originaldep }"/>
...
function btnsave(){
	var originaldep=document.getElementById("tMobilizepeople-originaldep").value;
	...
	if(originaldep=="机关生活服务中心"){//此处机关服务中心，也需要填写使用部门信息
		if(usedep==null||usedep==""){
			alert("请填写使用部门办理情况！");
		}else{
			if(servicedep==null||servicedep==""){
				alert("请填写机关生活服务中心办理情况！");
			}else{
				$.ajax({ 
			     	type:"POST", //表单提交类型 
			    	 url:'<%=basePath%>adminmessagecontroller/savepeoplechangesmsgofagencyservicecenterabouttwoparameter.htm', 
			     	data:{id:id,servicedep:servicedep,usedep:usedep}, //表单数据
			  		success:function(e){
				 	var  data = eval("("+e+")");
			      	if(data.status == 'ok'){//msg 是后台调用action时，你穿过来的参数
			       		alert(data.msg);
			       		location.href="<%=basePath%>adminmessagecontroller/findacceptmessagepageofagencyservicecenterbycondition.htm?depcode=${depcode}&currentPage=${currentPage}&title=${title}&sendpeople=${sendpeople}&status=${status}&senddepname=${senddepname}";
			     	 }else{
						alert(data.msg);
			     	 }
			     }
	...
	}else{//只需要填写机关服务中心处理意见
		if(servicedep==null||servicedep==""){
				alert("请填写机关生活服务中心办理情况！");
		}else{
		var originaldepexit;
		if(originaldep==null||originaldep==""){
			originaldepexit=false;
		}
		else{
			originaldepexit=true;
		}
		$.ajax({ 
	     	type:"POST", //表单提交类型 
	    	 url:'<%=basePath%>adminmessagecontroller/savepeoplechangesmsgofagencyservicecenteraboutoneparameter.htm', 
	     	data:{id:id,servicedep:servicedep,originaldepexit:originaldepexit}, //表单数据
	  		success:function(e){
		 	var  data = eval("("+e+")");
	      	if(data.status == 'ok'){//msg 是后台调用action时，你穿过来的参数
	       		alert(data.msg);
