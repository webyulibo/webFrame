  ...
  <body>
    <div class="container-f">
	  	 <div class="box-title clearfix tab-title">
			<h3>
				<a href="<%=basePath%>adminmessagecontroller/getsendmessagepageofpersonneldepartment.htm?depcode=03"><!-- 此处写死了部门代码是链接到部门代码为03 的  人事部门的发送通知页面 -->
				<span class="fa fa-table"></span>                                                                         <!--  注：如果认识部门的部门代码不是03了 ，人事部门发送通知的地方将会报错 -->
				发送通知
				</a>
			</h3>
			<h3>
				<a href="<%=basePath%>adminmessagecontroller/getacceptmessagepageofpersonneldepartment.htm?depcode=03"><!-- 此处写死了部门代码是链接到部门代码为03 的    人事部门的通知列表-->
  	...
  	<c:when test="${not empty TMobilizepeopleList}">
  		<table class="table-hover dataTable" style="min-width:1100px;">
  			...
  			<tr>		
  				...
  				<td>
  				<c:if test="${TMobilizepeopleList.status==true}">已处理完毕</c:if> 
				<c:if test="${TMobilizepeopleList.status==false}">未处理完毕</c:if>
				</td>
  				<td>
...
<script>
	...
	$('.dataTable tr:even').css('background','#F3F3F3');//黑白相间
