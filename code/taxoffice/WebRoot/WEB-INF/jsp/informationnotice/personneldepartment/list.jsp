...
<body onload="load()">
	<div class="container-f">
		 <div class="box-title clearfix tab-title">
			<h3>
				<a href="<%=basePath%>adminmessagecontroller/getsendmessagepageofpersonneldepartment.htm?depcode=${depcode }">
	...
	<cc:page url="${basePath}adminmessagecontroller/findacceptmessagepageofpersonneldepartmentbycondition.htm?depcode=${depcode }&status=${status }&title=${title }&sendpeople=${sendpeople }&senddepname=${senddepname }" value="${pager}" />
			</fieldset>
		</div>
	</div>
	...
	function findmsg(){
	 ...
	 location.href="<%=basePath%>adminmessagecontroller/findacceptmessagepageofpersonneldepartmentbycondition.htm?depcode="+depcode+"&status="+status+"&title="+title+"&sendpeople="+sendpeople+"&senddepname="+senddepname;
	}
	//查询通知的详情
	$(function(){
			...
			var depcode=document.getElementById("depcode").value;
	       	if(mark==true){//去查看t_mobilizepeople表
	       		location.href="<%=basePath%>adminmessagecontroller/getpersonchangesdetailpageofpersonneldepartment.htm?depcode="+depcode+"&id="+id+"&currentPage="+currentPage+"&status="+prestatus+"&title="+pretitle+"&sendpeople="+presendpeople+"&senddepname="+presenddepname;
	       	}else{//去查看普通通知表
	       		location.href="<%=basePath%>adminmessagecontroller/getacceptmessagedetailpageofpersonneldepartment.htm?depcode="+depcode+"&id="+id+"&currentPage="+currentPage+"&status="+prestatus+"&title="+pretitle+"&sendpeople="+presendpeople+"&senddepname="+presenddepname;
	       	}
	    	});
	    });
	    //删除对应的通知
			...
			var depcode=document.getElementById("depcode").value;
	       	if(mark==true){//去查看t_mobilizepeople表
	       		$.ajax({ 
					     	type:"POST", //表单提交类型 
					    	 url:'<%=basePath%>adminmessagecontroller/deletepeoplechangemessageofpersonneldepartment.htm', 
					     	data:{id:id,currentPage:currentPage,depcode:depcode,title:pretitle,sendpeople:presendpeople,senddepname:presenddepname,status:prestatus}, //表单数据
					      	...
					      	if(data.status == 'ok'){//msg 是后台调用action时，你穿过来的参数
					      		 ...
					      		 location.href="<%=basePath%>adminmessagecontroller/findacceptmessagepageofpersonneldepartmentbycondition.htm?depcode="+depcode+"&status="+prestatus+"&title="+pretitle+"&sendpeople="+presendpeople+"&senddepname="+presenddepname+"&currentPage=${pager.currentPage-1}";
					     	 	}else{
					     	 	  location.href="<%=basePath%>adminmessagecontroller/findacceptmessagepageofpersonneldepartmentbycondition.htm?depcode="+depcode+"&status="+prestatus+"&title="+pretitle+"&sendpeople="+presendpeople+"&senddepname="+presenddepname+"&currentPage=${pager.currentPage}";
					     	 	}
					     	 }else{
								alert(data.msg);
					     	 }
	       		...
	       		$.ajax({ 
					     	type:"POST", //表单提交类型 
					    	 url:'<%=basePath%>adminmessagecontroller/deleteacceptmessagepageofpersonneldepartment.htm', 
					     	data:{id:id,currentPage:currentPage,depcode:depcode,title:pretitle,sendpeople:presendpeople,senddepname:presenddepname,status:prestatus}, //表单数据
					  		success:function(e){
						 	 var  data = eval("("+e+")");
					      	if(data.status == 'ok'){//msg 是后台调用action时，你穿过来的参数
