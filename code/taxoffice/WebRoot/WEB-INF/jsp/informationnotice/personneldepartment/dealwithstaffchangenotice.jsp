	...
	<div class="container-f">
	  	 <div class="box-title clearfix tab-title"> <!-- 标题 -->
			...
			<div style=" float:left;margin-left:20px;">
				<a href="<%=basePath%>adminmessagecontroller/findacceptmessagepageofpersonneldepartmentbycondition.htm?depcode=${depcode}&currentPage=${currentPage}&title=${title}&sendpeople=${sendpeople}&status=${status}&senddepname=${senddepname}" class="btn btn-primary ">返回</a>
		   </div>
		</div>
...
function btnsave(){
	...
	if(servicedep==null||servicedep==""){
		alert("请填写使用部门办理情况！");
			...
			$.ajax({ 
	     	type:"POST", //表单提交类型 
	    	 url:'<%=basePath%>adminmessagecontroller/savepeoplechangesmsgofpersonneldepartment.htm', 
	     	data:{id:id,userdep:userdep,depcode:depcode}, //表单数据
	  		success:function(e){
		 	var  data = eval("("+e+")");
	      	if(data.status == 'ok'){//msg 是后台调用action时，你穿过来的参数
	       		alert(data.msg);
	       		location.href="<%=basePath%>adminmessagecontroller/findacceptmessagepageofpersonneldepartmentbycondition.htm?depcode=${depcode}&currentPage=${currentPage}&title=${title}&sendpeople=${sendpeople}&status=${status}&senddepname=${senddepname}";
	     	 }else{
				alert(data.msg);
	     	 }
	     }
