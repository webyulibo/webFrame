	...
	<div class="container-f">
	  	 <div class="box-title clearfix tab-title"> <!-- 标题 -->
			...
			<div style=" float:left;margin-left:20px;">
				<a href="<%=basePath%>adminmessagecontroller/findacceptmessagepageoffinancialmanagementbycondition.htm?depcode=${depcode}&currentPage=${currentPage}&title=${title}&sendpeople=${sendpeople}&status=${status}&senddepname=${senddepname}" class="btn btn-primary ">返回</a>
		   </div>
		</div>
	...
	<form action="" name="return">
		<fieldset class="search" style="margin-top:20px;">
			...
			<c:if test="${tMobilizepeople.status==false }"> 
				<td width="130px" height="140px"><center>财务部门办理情况：</center></td>
				<td colspan="3" height="140px"><textarea  id="financialdep" style="width:100%;height:100%" >${tMobilizepeople.financialdep }</textarea></td>
			</c:if>
			</tr>
			<tr>
				 <c:if test="${tMobilizepeople.status==true }"> 
				...
				<td colspan="3" height="140px"><textarea id="usedep" style="width:100%;height:100%" readonly>${tMobilizepeople.usedep }</textarea></td>
				 ...
				 <c:if test="${tMobilizepeople.status==false }"> 
				 <c:if test="${tMobilizepeople.originaldep=='财务管理科' }"> 
				 <td width="130px" height="140px"><center>使用部门办理情况：</center></td>
...
function btnsave(){
	...
	if(originaldep=="财务管理科"){//此处机关服务中心，也需要填写使用部门信息
		if(financialdep==null||financialdep==""){
			alert("请填写财务部门办理情况！");
		}else{
			if(usedep==null||usedep==""){
				...
				$.ajax({ 
			     	type:"POST", //表单提交类型 
			    	 url:'<%=basePath%>adminmessagecontroller/savepeoplechangesmsgoffinancialmanagementabouttwoparameter.htm', 
			     	data:{id:id,financialdep:financialdep,usedep:usedep}, //表单数据
			  		success:function(e){
				 	var  data = eval("("+e+")");
			      	if(data.status == 'ok'){//msg 是后台调用action时，你穿过来的参数
			       		alert(data.msg);
			       		location.href="<%=basePath%>adminmessagecontroller/findacceptmessagepageoffinancialmanagementbycondition.htm?depcode=${depcode}&currentPage=${currentPage}&title=${title}&sendpeople=${sendpeople}&status=${status}&senddepname=${senddepname}";
			     	 }else{
						alert(data.msg);
			     	 }
			     }
		...
		$.ajax({ 
	     	type:"POST", //表单提交类型 
	    	 url:'<%=basePath%>adminmessagecontroller/savepeoplechangesmsgoffinancialmanagementaboutoneparameter.htm', 
	     	data:{id:id,financialdep:financialdep,originaldepexit:originaldepexit}, //表单数据
	  		success:function(e){
		 	var  data = eval("("+e+")");
	      	if(data.status == 'ok'){//msg 是后台调用action时，你穿过来的参数
	       		alert(data.msg);
