...
<script type="text/javascript">
	...
	function submit() {
		...
		var filepath1=document.getElementById("searchBtn").getAttribute("filepath");
		...
		$.ajax({ 
		    ...
		    type:"POST", //表单提交类型 
		     ...
		     data:{resultnum:resultnum1,checkpeople:checkpeople1,remark:remark1,filePath:filepath1}, //表单数据
		     success:function(e){
...
<body>
	<div class="container-f">
		...
		<div class="clearfix">
			...
			<%-- <c:if test="${not empty message }">
				${message }
							...
							<td><input type="button" id="searchBtn"  filepath="${filePath }"   value="提交"
		...
		<div style="margin-top:10px;" id="list">
			...
			<c:choose>
				<c:when test="${not empty assetsCheckeds && not empty assetsNotCheckeds }">
					<table id="tableinfo" class="table-hover dataTable" style="margin-top:10px;">
							<tr>
				...
				<c:choose>
					...
					<c:choose>
						...
						<table id="tableinfo"  class="table-hover dataTable">
							...
							<c:forEach items="${assetsCheckeds }" var="assetsChecked" varStatus="status">
								<tr>
									<%count++; %>
									<td><%=count %></td>
									<td>${assetsChecked.code }</td>
		...
		<c:if test="${ not empty assetCustomUnusuals }">
				<div style="float:right;padding:10px;">
					<a class="btn cf" href="<%=basePath%>adminassetcheckresultimportcontroller/importunusualassettoexcel.htm" id="search1"   ><i class="fa fa-edit "></i>导出</a>
				</div>
		...
		<table class="table-hover dataTable">
  			...
  			<c:forEach items="${ assetCustomUnusuals}"  var="assetCustomUnusual" varStatus="status" >
  			<tr>
				<td>${status.index+1 }</td>
				<td>${assetCustomUnusual.code }</td>
				<td>${assetCustomUnusual.name }</td>
				<td>${assetCustomUnusual.originaluser }</td>
				<td>${ assetCustomUnusual.user}</td>
				<td>${assetCustomUnusual.originaldepartment }</td>
				<td>${assetCustomUnusual.usedepartment }</td>
				<%-- <td>${assetCustomUnusual.unusualremark }</td> --%>
			</tr>
		        	...
		        	editor.loadPlugin('image', function () {
						editor.plugin.imageDialog({
							...
							clickFn: function (url, title, width, height, border, align) {
								...
								location.href="<%=basePath%>adminassetcheckresultimportcontroller/importassetcheckresulttotextshow.htm?filePath="+filepath;
	...
	<shiro:hasPermission name="checkauth">
		...
		$("#searchBtn").attr("onclick","alert('权限不足，无法进行此操作')");
