...
<html>
  ...
  <div class="returnbtn">
			...
			${fAssetTypeName }
			 ...
			 <br />
			 <br />
				<a href="javascript:void(0)" class="btn btn-primary" onclick="f()" >返回</a>
				
  </div>
   <form id="asset" action=" " method="post" class="form" >
		...
		<div class="control-group cf clearfix " >
					<label class="control-label" for="web_bg">
						...
						<input type="text" name=" " id="value"  class="input" value=""style="padding: 5px 2px; " placeholder="小数点后最多保留两位" onfocus="javascript:placeholder=''" onBlur="javascript:placeholder='小数点后最多保留两位'">
...
function addassettypemenu(){
	...
	if(!isNaN(Value)){
     ...
     data:{fid:${id},typename:Typename,unit:Unit,value:Value}, //表单数据
  success:function(e){
	  var  data = eval("("+e+")");
      if(data.status == 'ok'){//msg 是后台调用action时，你穿过来的参数
       		alert(data.msg);
       		
       			location.href="<%=basePath%>adminassettype/getassettypeelsemenubyfidwithpage.htm?id=${id}";
